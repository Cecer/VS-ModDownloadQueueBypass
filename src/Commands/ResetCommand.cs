...
internal class ResetCommand(Config config, BypassTicketManager ticketManager, RequeueReliefEventHandler queueEventHandler) : Command("reset")
{
    ...
    [
        new Queue(queueEventHandler),
        new Tickets(ticketManager)
    ];

    private class Queue(RequeueReliefEventHandler queueEventHandler) : Command("queue")
    {
        ...
        {
            var queueSize = queueEventHandler.Queue.QueuePopulation;
            queueEventHandler.Queue.RemoveAll("Queue reset via command");
